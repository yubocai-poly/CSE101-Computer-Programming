...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 13 16:50:57 2022
...
class GameGrid:
    ...
    def neighbors(self, i, j):
        ...
        return [x for x in neighbors_set if self.is_inside(x)]
    ...
    def reveal(self, i, j):
        ...
        if self.mines_nearby(i, j) == 0 and not self.square_at(i, j).mined:
            neighbors = self.neighbors(i, j)
            for square in neighbors:
                u, v = square
                self.reveal(u, v)
    ...
    def chording(self, i, j):
        ...
        if self.flags_nearby(i, j) == self.mines_nearby(i, j):
            ...
            for square in neighbors:
                i, j = square
                if not self.square_at(i, j).flagged:
                    self.reveal(i, j)
    ...
    def random_game(self, n_mines):
        ...
        for _ in range(n_mines):
            ...
            while self.square_at(i, j).mined:
                i, j = random.randint(0, self.nrows - 1), random.randint(0, self.ncols - 1)
            self.place_mine(i, j)
...
class MinesweeperApp:
    ...
    def __init__(self, game):
        self.game = game
        self.undetected_mines = game.nmines
        self.flags_left = game.nmines
        self.started = False
    ...
    def update_mine_counter(self):
        ...
        mines = str(self.game.nmines - self.game.nflags)
        self.mine_counter_str.set(mines)
    ...
    def update_time_counter(self):
        ...
        self.time_counter_str.set(str(self.game_time()))
    ...
    def left_click_handler(self, i, j):
        ...
        if self.game.square_at(i, j).revealed:
            if not self.game.square_at(i, j).mined:
    ...
    def right_click_handler(self, i, j):
        ...
        if not self.game.square_at(i, j).revealed:
            if self.game.square_at(i, j).flagged:
                self.game.square_at(i, j).flagged = False
                self.game.nflags -= 1
            else:
                self.game.square_at(i, j).flagged = True
                self.game.nflags += 1
            self.draw_board()
            if self.game.game_won():
                self.game_over(True)
