...
class GameGrid:
    ...
    def str_for_player(self):
        ...
        lines = [
            ''.join([sq.str_for_player() for sq in line]) for line in self.tab
        ]
        return '\n'.join(lines)
    ...
    def neighbors(self, i, j):
        l = []
        neighbors_set = [(i + 1, j), (i, j + 1), (i - 1, j), (i, j - 1),
                         (i + 1, j + 1), (i - 1, j - 1), (i + 1, j - 1),
                         (i - 1, j + 1)]
        for el in neighbors_set:
            if el[0] < self.nrows and el[1] < self.ncols and el[0] >= 0 and el[
                    1] >= 0:
                l.append(el)
        return l
    ...
    def place_mine(self, i, j):
        ...
        """
        if self.mined(i, j) is True:
            return None

        if self.mined(i, j) is False:
            self.square_at(i, j).mined = True
            self.nmines += 1
            neighbor = self.neighbors(i, j)
            for square in neighbor:
                i = square[0]
                j = square[1]
                self.square_at(i, j).mines_nearby += 1
    ...
    def reveal(self, i, j):
        ...
        """If the square has already been revealed or is flagged, nothing happens
        Otherwise, the square is revealed, and self.squares_revealed is incremented"""
        if self.mines_nearby(i, j) == 0 and self.mined(i, j) is False:
            neighbor = self.neighbors(i, j)
            for square in neighbor:
                i = square[0]
                j = square[1]
    ...
    def game_won(self):
        ...
        """
        return (not self.game_lost()) and (
            self.nrows * self.ncols - self.squares_revealed) == self.nmines
...
class MinesweeperApp:
    ...
    def update_mine_counter(self):
        ...
        mines = self.game.nmines - self.game.nflags
        self.mine_counter_str.set(str(mines))
        self.top_frame.after(100, self.update_mine_counter)
        # The logic from this question is from Junyuan

    # EXERCISE 8
    def update_time_counter(self):
        """Updated the time counter and schedules the next update in 100ms"""
        input = time.time() - self.t_0
        self.time_counter_str.set(int(input))
    ...
    def left_click_handler(self, i, j):
        ...
        if self.game.flagged(i, j) is True:
            return None
        """If the cell at (i,j) has a flag on it, then left_click_handler(i,j) does nothing. 
        (For the moment there aren’t any flags, but that will change soon!)"""

        if self.game.revealed(i, j):
            self.game.chording(i, j)
        else:
            self.game.reveal(i, j)
        """If the cell at (i,j) has already been revealed, then left_click_handler(i, j) calls the chording method of self.game
        Otherwise, left_click_handler(i,j) reveals the contents of the cell using the reveal method of self.game."""
        ...
        if self.game.game_won():
            self.game_over(True)
        """If the player lost the game (use game_lost method of GameGrid), we call game_over(False) to announce that the player has lost.
        If the player won the game (use game_won of GameGrid), we call game_over(True) to tell the player the good news."""
    ...
    def right_click_handler(self, i, j):
        ...
        if self.game.revealed(i, j) is True:
            return None
        """If you right-click on a square that has already been revealed, then nothing happens."""

        if self.game.flagged(i, j) is False:
            self.game.place_flag(i,j)
        else:
            self.game.remove_flag(i,j)
        """If you right-click on a square that has not been revealed, and has no flag, 
        then the corresponding square in self.game is flagged."""

        """If you right-click on a square that has not been revealed but that already has a flag on it, 
        then the corresponding square in self.game has its flag removed."""
